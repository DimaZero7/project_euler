...
for row in range(1, count_rows + 1):
    end_row = elements_in_row * row
    start_row = end_row - elements_in_row

    for element in range(start_row, end_row):

        # If we reach the end of the row, then skip multiplication
        ...
        max_product = max(max_product, vertical_straight_product, horizontal_straight_product, diagonal_left_product, diagonal_right_product)
